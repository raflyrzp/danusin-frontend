...
export default function EditProductPage() {
  ...
  useEffect(() => {
    if (product) {
      reset({
        ...
        available_days: DAYS,
        pickup_locations: LOCATIONS,
        po_open_date: product.po_open_date?.split("T")[0] || "",
        po_close_date: product.po_close_date?.split("T")[0] || "",
      });
      ...
      if (product.images && product.images.length > 0) {
        const existingImages: ImagePreview[] = product.images.map(
          (img: any) => ({
            id: `existing-${img.id}`,
            preview: getImageUrl(img.url) || img.url,
            isExisting: true,
          })
        );
    ...
    setImageFiles((prev) => {
      ...
      if (imageToRemove) {
        if (!imageToRemove.isExisting) {
          ...
          const existingId = parseInt(id.replace("existing-", ""));
          if (!isNaN(existingId)) {
  ...
  const onSubmit = async (data: EditProductFormData) => {
    try {
      ...
      // Upload new images
      const newImageFiles = imageFiles.filter((img) => !img.isExisting);
      let uploadedImageUrls: string[] = [];

      if (newImageFiles.length > 0) {
        const files = newImageFiles.map((img) => img.file!);
      ...
      await updateProduct.mutateAsync({
        ...
        data: {
          ...
          add_images: uploadedImageUrls.length > 0 ? uploadedImageUrls : undefined,
          remove_image_ids: removedImageIds.length > 0 ? removedImageIds : undefined,
        },
      });
  ...
  const isSubmitting = updateProduct.isPending || isUploading;
    ...
    return (
      <div className="rounded-xl border border-dashed border-red-300 bg-red-50 p-10 text-center">
        ...
        <Button asChild className="mt-4">
          <Link href="/store/products">Kembali</Link>
        </Button>
      </div>
    );
  }
  ...
  return (
    <div className="max-w-2xl">
      ...
      <div className="rounded-xl border border-[#E3D9BD] bg-white p-6 shadow-sm">
        <h1 className="text-xl font-bold text-[#4E1F00] mb-6">Edit Produk</h1>
